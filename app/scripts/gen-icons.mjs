// Generates Mero Meet's icon set into app/public/ with zero dependencies:
//   favicon.ico (16/32/48 PNG-compressed entries), apple-touch-icon.png,
//   icon-192.png, icon-512.png.
// The mark — a camcorder body plus lens wedge in the app's accent gradient — is
...
// ---- art -------------------------------------------------------------------
// The camera silhouette, filled with --accent-grad (#6d83ff → #a06bff) so the
// icon and the app's buttons are visibly the same brand.

const BG = "#090b10";

function mark(c, fit) {
  const g = grad(9, 20, 56, 45, "#6d83ff", "#a06bff");
  draw(c, rrect(9, 20, 32, 24, 7), g, fit);
  draw(c, poly([[44, 26], [56, 19], [56, 45], [44, 38]]), g, fit);
}
